...
$query = mysqli_query($conn, "SELECT * FROM tickets WHERE user = '$user' AND UID = '$ticketid'");

$query2 = mysqli_query($conn, "SELECT * FROM users WHERE user = '$user' AND active = '2'");
...
if (isset($_POST['ticketanswersubmit'])) {
	...
	$ticketanswertext = htmlentities($ticketanswertext);
	$ticketanswertext = mysqli_real_escape_string($conn, $ticketanswertext);
				...
				<?php  
						...
						while ($row = mysqli_fetch_assoc($query)) {
							...
							if ($row['user'] == $_SESSION['username']) {
								echo '<div id="ticketanswer">' . $row['text'] .   ' <br> <a href="profile.php?ID=' . $id . '">'  . $row['user'] . '</a></div><br>';
							} else {
